...
type Api = {
  store: any
  activateSubRule: ReturnType<typeof registerRule>['activateSubRule']
  removeRuleFromRuleDB: ReturnType<typeof createRuleDB>['removeRule']
}

export default function createConsequenceFn (api:Api) {
  let execId = 1
  let wrappedExecIds = []

  const getCurrentRuleExecId = () => wrappedExecIds[wrappedExecIds.length-1] || null
  ...
  function consequence (actionExecution:t.ActionExecution, ruleContext:t.RuleContext) {
    ...
    const addRule = (name, parameters) => {
      api.activateSubRule(ruleContext, name, parameters)
    }
    const removeRule = name => {
      const context = ruleContext.subRuleContexts[name]
      if(!context || !context.active) return
      api.removeRuleFromRuleDB(context)
    }

    const consequenceArgs = {
      addRule, 
      removeRule, 
      effect, 
      wasCanceled, 
    ...
    // register unlisten callback
      ...
      const offCancel = ruleContext.events.once('CANCEL_CONSEQUENCE', (newRuleExecution:t.RuleExecution) => {
        if(newRuleExecution.concurrencyId !== ruleExecution.concurrencyId) return
        if(newRuleExecution.execId === ruleExecution.execId) return
        offRemoveRule()
        result()
  ...
  return {consequence, getCurrentRuleExecId}
}